...
# Project-specific cmake options can be passed through without this shared
# script needing to know about them, e.g.
#   EXTRA_CMAKE_ARGS=-DPEANUT_FULL_GBC_SUPPORT=OFF ./bld.sh -c 1
[ -n "$EXTRA_CMAKE_ARGS" ] && echo "Extra cmake args: $EXTRA_CMAKE_ARGS"
if [ -z "$TOOLCHAIN_PATH" ] ; then
	cmake -DCMAKE_BUILD_TYPE=$BUILD -DPICO_BOARD=$PICO_BOARD -DHW_CONFIG=$HWCONFIG -DPICO_PLATFORM=$PICO_PLATFORM -DENABLE_PIO_USB=$USEPIOUSB -DUSE_PICO_EXTRAS_I2S=$USEEXTRASI2S -DBUILD_FOR_BOOTLOADER=$USEBOOTLOADER $FORCEDVIOPT $EXTRA_CMAKE_ARGS .. || exit 1
else
	cmake -DCMAKE_BUILD_TYPE=$BUILD -DPICO_BOARD=$PICO_BOARD -DHW_CONFIG=$HWCONFIG -DPICO_PLATFORM=$PICO_PLATFORM -DENABLE_PIO_USB=$USEPIOUSB -DUSE_PICO_EXTRAS_I2S=$USEEXTRASI2S -DBUILD_FOR_BOOTLOADER=$USEBOOTLOADER -DPICO_TOOLCHAIN_PATH=$TOOLCHAIN_PATH $FORCEDVIOPT $EXTRA_CMAKE_ARGS .. ||  exit 1
fi
