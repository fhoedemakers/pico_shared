...
extern const unsigned char samplesound_wav[]; // generated in soundrecorder.cpp

#ifndef MENU_WAV_START_OFFSET_SEC
#define MENU_WAV_START_OFFSET_SEC 0.0f
#endif
...
static MenuWavState g_menu_wav{};
...
static void menu_wav_apply_offset()
{
    if (!g_menu_wav.ready) return;
    if (g_menu_wav.sample_rate == 0) return;

    uint64_t start_frames = (uint64_t)(g_menu_wav.offset_sec * (float)g_menu_wav.sample_rate + 0.5f);
    if (g_menu_wav.kind == WavSrcKind::Memory) {
        if (g_menu_wav.frames_total == 0) return;
        g_menu_wav.start_frame = (uint32_t)(start_frames % g_menu_wav.frames_total);
        g_menu_wav.frame_pos   = g_menu_wav.start_frame;
    } else { // File
        uint64_t start_bytes = start_frames * g_menu_wav.bytes_per_frame;
        if (g_menu_wav.data_bytes == 0) return;
        start_bytes %= g_menu_wav.data_bytes;
        g_menu_wav.stream_pos_bytes = (uint32_t)start_bytes;
        g_menu_wav.frame_pos        = (uint32_t)(start_bytes / g_menu_wav.bytes_per_frame);
        g_menu_wav.start_frame      = g_menu_wav.frame_pos;
        f_lseek(&g_menu_wav.fil, g_menu_wav.data_start + g_menu_wav.stream_pos_bytes);
    }
}

void menu_wav_set_offset_seconds(float seconds)
{
    g_menu_wav.offset_sec = seconds < 0.0f ? 0.0f : seconds;
    menu_wav_apply_offset();
}

// Initialize from embedded memory array
static void menu_wav_init_memory()
{
    const unsigned char *wav = samplesound_wav;
    if (!(wav[0]=='R'&&wav[1]=='I'&&wav[2]=='F'&&wav[3]=='F')) { g_menu_wav.ready=false; return; }
    if (!(wav[8]=='W'&&wav[9]=='A'&&wav[10]=='V'&&wav[11]=='E')) { g_menu_wav.ready=false; return; }
    ...
    for (;;) {
        ...
        p += 8 + sz;
        if (p > 0x1000) { g_menu_wav.ready=false; return; } // simple guard
    }
    ...
    if (audio_format != 1 || channels != 2 || bits_per != 16 || sample_rate == 0 || data_size < 4) { g_menu_wav.ready=false; return; }

    g_menu_wav.kind = WavSrcKind::Memory;
    g_menu_wav.sample_rate = sample_rate;
    g_menu_wav.bytes_per_frame = 4;
    g_menu_wav.pcm_mem = (const int16_t*)(&wav[data_off]);
    g_menu_wav.frames_total = data_size / 4;
...
// Initialize from file path (FatFs), returns true if prepared
static bool menu_wav_use_file(const char* path)
{
    ...
    UINT rd = 0;
    fr = f_read(&g_menu_wav.fil, hdr, sizeof(hdr), &rd);
    if (fr != FR_OK || rd < 44) { f_close(&g_menu_wav.fil); return false; }

    if (!(hdr[0]=='R'&&hdr[1]=='I'&&hdr[2]=='F'&&hdr[3]=='F')) { f_close(&g_menu_wav.fil); return false; }
    if (!(hdr[8]=='W'&&hdr[9]=='A'&&hdr[10]=='V'&&hdr[11]=='E')) { f_close(&g_menu_wav.fil); return false; }
    ...
    if (!fmt_off || !data_off) { f_close(&g_menu_wav.fil); return false; }
    ...
    if (audio_format != 1 || channels != 2 || bits_per != 16 || sample_rate == 0 || data_size < 4) { f_close(&g_menu_wav.fil); return false; }

    g_menu_wav.kind = WavSrcKind::File;
    g_menu_wav.sample_rate = sample_rate;
    g_menu_wav.bytes_per_frame = 4;
    g_menu_wav.data_start = data_off;
    g_menu_wav.data_bytes = data_size;
    g_menu_wav.frames_total = data_size / 4; // informational
    g_menu_wav.stream_pos_bytes = 0;
    g_menu_wav.frame_pos = 0;
    g_menu_wav.start_frame = 0;
    g_menu_wav.ready = true;

    // Seek to beginning to allow lseek to data later
    f_lseek(&g_menu_wav.fil, 0);
    menu_wav_apply_offset();
    printf("Menu WAV file: %s, %u Hz, %u bytes\n", path, sample_rate, data_size);
    return true;
}

// Pump frames. For file source, read small chunks, loop to offset when hitting EOF
static void menu_wav_pump(uint32_t frames_to_push)
{
    if (!g_menu_wav.ready) return;
    ...
    if (g_menu_wav.kind == WavSrcKind::Memory) {
        while (frames_to_push--) {
            const int16_t* p = g_menu_wav.pcm_mem + (g_menu_wav.frame_pos * 2);
            int16_t l = p[0], r = p[1];
            EXT_AUDIO_ENQUEUE_SAMPLE(l, r);
            g_menu_wav.frame_pos++;
            if (g_menu_wav.frame_pos >= g_menu_wav.frames_total) {
                g_menu_wav.frame_pos = g_menu_wav.start_frame; // loop back to offset
            }
        }
    } else {
        // Read chunked frames from file
        // Small buffer on stack for I/O
        int16_t buf[256 * 2]; // 256 frames stereo
        while (frames_to_push) {
            uint32_t want_frames = std::min<uint32_t>(frames_to_push, 256);
            uint32_t bytes_avail = g_menu_wav.data_bytes - g_menu_wav.stream_pos_bytes;
            if (bytes_avail == 0) {
                // Loop to offset
                g_menu_wav.stream_pos_bytes = g_menu_wav.start_frame * g_menu_wav.bytes_per_frame;
                f_lseek(&g_menu_wav.fil, g_menu_wav.data_start + g_menu_wav.stream_pos_bytes);
                bytes_avail = g_menu_wav.data_bytes - g_menu_wav.stream_pos_bytes;
            }
            uint32_t bytes_to_read = std::min<uint32_t>(want_frames * g_menu_wav.bytes_per_frame, bytes_avail);
            UINT rd = 0;
            FRESULT fr = f_read(&g_menu_wav.fil, buf, bytes_to_read, &rd);
            if (fr != FR_OK || rd == 0) {
            ...
            g_menu_wav.stream_pos_bytes += rd;

            uint32_t got_frames = rd / g_menu_wav.bytes_per_frame;
            const int16_t* p = buf;
            ...
            g_menu_wav.frame_pos += got_frames;
            frames_to_push -= got_frames;

            // If we ended exactly at EOF, next loop will reset to offset
            if (g_menu_wav.stream_pos_bytes >= g_menu_wav.data_bytes) {
                g_menu_wav.stream_pos_bytes = g_menu_wav.start_frame * g_menu_wav.bytes_per_frame;
                f_lseek(&g_menu_wav.fil, g_menu_wav.data_start + g_menu_wav.stream_pos_bytes);
                g_menu_wav.frame_pos = g_menu_wav.start_frame;
            }
...
#if HW_CONFIG == 8
        if (g_menu_wav.ready) {
            menu_wav_pump(g_menu_wav.sample_rate / 60);
        }
#endif
